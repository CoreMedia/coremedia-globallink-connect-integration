...
:::warning
If the major releases are not approved in strict chronological order, the
sorting of the versions in `website/versions.json` must be adjusted manually
after creating a snapshot.

For example, if you created the version snapshot for `2406.x` after the
snapshot for `2412.x`, apply this change:
