...
final class Task {
   ...
   */
  Task(String content, Locale targetLocale, TaskState... taskStates) {
    this.content = content;
    this.targetLocale = targetLocale;
    long currentTimeMillis = System.currentTimeMillis();
    if (taskStates == null || taskStates.length == 0) {
      timeInMillisToState.put(currentTimeMillis, TaskState.COMPLETED);
    } else {
      for (TaskState taskState : taskStates) {
        timeInMillisToState.put(currentTimeMillis, taskState);
      }
    }
