...
@NullMarked
class TranslateInvalidContentIdScenarioTest {
  private static final Logger LOG = getLogger(lookup().lookupClass());

  @Nested
  class FacadeIntegrationBehavior {
    @Test
    void shouldMockImportWithInvalidIdSyntax(TestInfo testInfo) {
      ...
      GCExchangeFacade facade = new MockedGCExchangeFacade(MockSettings.fromMockConfig(
        Map.of(
          ...
          MockSettings.SCENARIO, TranslateInvalidContentIdScenario.ID
        )
      ));
      ...
      assertThat(xliffResult)
        ...
        .matches("(?s).*cmxliff:target=\"invalid:\\d+\".*")
      ;
    }
  }
}
