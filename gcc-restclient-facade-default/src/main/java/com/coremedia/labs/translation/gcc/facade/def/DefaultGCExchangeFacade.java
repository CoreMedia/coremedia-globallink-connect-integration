...
public class DefaultGCExchangeFacade implements GCExchangeFacade {
  ...
  private void confirmTaskCancellations(List<Long> taskIds) {
    ...
    List<TasksConfirmCancellation.TaskConfirmCancellationFailure> confirmationsFailures;
    try {
      confirmationsFailures = delegate.confirmTaskCancellation(taskIds);
    } catch (RuntimeException e) {
      throw new GCFacadeCommunicationException(e, "Failed to confirm cancelled tasks: " + taskIds);
    }

    if (confirmationsFailures != null && !confirmationsFailures.isEmpty() && LOG.isDebugEnabled()) {
      Map<Long, List<String>> messagesByTaskId = confirmationsFailures.stream()
              .collect(
                      groupingBy(
                              TasksConfirmCancellation.TaskConfirmCancellationFailure::getTaskId,
                              mapping(TasksConfirmCancellation.TaskConfirmCancellationFailure::getMessage, toList())
                      )
              );
      LOG.debug("Failed to confirm task cancellation. Will retry. Failed confirmation information: {}", messagesByTaskId);
    }
  }
  ...
  private Map<TaskStatus, Set<GCTaskModel>> getTasksByState(long submissionId, TaskStatus... taskStates) {
    return getTasksByState(submissionId, r -> {}, taskStates);
  }
  ...
  public GCSubmissionState getSubmissionState(long submissionId) {
    ...
    return GCSubmissionState.fromSubmissionState(submission.getState());
  }
